...
const PERMISSIONS = [
  { key: "can_view_admin_sellers", labelPl: "Lista", labelEn: "List" },
  { key: "can_impersonate_sellers", labelPl: "Impersonacja", labelEn: "Impersonate" },
  { key: "can_grant_admin_permissions", labelPl: "Grant", labelEn: "Grant" },
] as const;
...
export default function AdminSellersPage() {
  ...
  const updatePermission = async (seller: Seller, key: PermissionKey, checked: boolean) => {
      ...
      <section className="overflow-hidden rounded-[1.5rem] shadow-sm" style={{ background: "var(--bg-card)", border: "1px solid var(--border-default)" }}>
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-[1040px] w-full text-left text-sm" style={{ borderCollapse: "collapse" }}>
              <thead className="text-xs uppercase tracking-[0.2em]" style={{ background: "var(--bg-table-header)", color: "var(--text-secondary)" }}>
                <tr>
                  ...
                  <th className="px-4 py-3 font-semibold">{copy.role}</th>
                  <th className="px-4 py-3 font-semibold">{copy.status}</th>
                  <th className="px-4 py-3 font-semibold">{copy.verified}</th>
                  <th className="px-4 py-3 font-semibold">{copy.lastLogin}</th>
                  <th className="px-4 py-3 font-semibold">{copy.permissions}</th>
              ...
              <tbody>
                {sellers.map((seller) => {
                  ...
                  const canImpersonate = Boolean(currentUser && canImpersonateSeller(currentUser, seller));

                  return (
                    <tr key={seller.id} style={{ borderTop: "1px solid var(--border-default)" }}>
                      <td className="px-4 py-4 align-top">
                        <div className="font-semibold" style={{ color: "var(--text-primary)" }}>{seller.name || seller.email || `#${seller.id}`}</div>
                        <div className="mt-1 text-xs" style={{ color: "var(--text-tertiary)" }}>{seller.email}</div>
                      </td>
                      ...
                      <td className="px-4 py-4 align-top">
                        <div className="flex flex-wrap gap-3">
                          {PERMISSIONS.map((permission) => {
                            ...
                            return (
                              <label key={permission.key} className={`inline-flex items-center gap-2 rounded-lg border px-2.5 py-1.5 text-xs font-semibold transition ${
                                locked ? "cursor-not-allowed opacity-60" : "cursor-pointer hover:bg-white/5"
                              }`} style={{ borderColor: "var(--border-default)", color: "var(--text-secondary)" }}>
                                <input
                                  type="checkbox"
                                  checked={seller[permission.key]}
                                  readOnly={locked}
                                  ...
                                  className="h-4 w-4 rounded border-slate-500 bg-slate-950 accent-indigo-500"
                                />
                                {lang === "pl" ? permission.labelPl : permission.labelEn}
                              </label>
                            );
                          })}
                        </div>
